...
Usage: `lean --run export_json.lean` creates an `export.json` file in the current directory.
-/
...
meta def export_json (filename : string) : io unit := do
handle ← mk_file_handle filename mode.write,
json ← run_tactic mk_export_json,
put_str handle json.unparse,
close handle
