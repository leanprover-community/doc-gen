...
def test_plaintext_summary():
    ...
    * `division_algebra ℍ[R]` : for a linear ordered field `R`.""")
    expected = (
        ...
        "define some algebraic structures on `ℍ[R]`. Main definitions. `quaternionalgebra R a b`, "
        "`ℍ[R, a, b]` :. quaternion algebra with coefficients `a`, `b`. `quaternion R`, `ℍ[R]` : "
        "the space of quaternions, a.k.a. `quaternionalgebra R (-1) (-1)`;. `quaternion.normsq` : "
        ...
        ": for any commutative ring `R`;. `domain ℍ[R]` : for a linear ordered commutative ring "
        "`R`;. `division_algebra ℍ[R]` : for a linear ordered field `R`.")
